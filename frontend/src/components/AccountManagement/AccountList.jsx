...
const AccountList = () => {
    ...
    const deleteStorageInfo = (id) => {
        const newStorageState = accountState.filter(
    ...
    return (
        <div className="max-w-full flex-grow">
            ...
            <div className="relative justify-items-center mx-10 mt-2 p-4">
                ...
                <div id='AccountList' className="rounded-lg border-2 border-gray-300 shadow-md px-4 py-5 mb-8">
                    {accountState.map(storageInfo => (
                        <div key={storageInfo.id}>
                            {/* <Link to={`./${storageInfo.id}`}></Link>  */}
                            <StorageCard storageProps={storageInfo} deleteFunc={deleteStorageInfo} editFunc={handleEditStorageInfo} key={storageInfo.id}/>
                        </div>
                    ))}
                </div>
