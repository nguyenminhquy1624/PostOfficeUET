...
const AddStorageStaffModal = (props) => {
  ...
  const handleAccountNameDown = (event) => {
    if (event.key === "Enter") {
      event.preventDefault();
      if (accountName.trim() !== "") accountPhoneNumberRef.current.focus();
    }
  };
  ...
  const handleAccountPhoneNumberDown = (event) => {
    if (
      !phoneNumberRegex.test(accountPhoneNumber) &&
  ...
  return (
    ...
    >
      {/* AddPage Modal */}
      <div
        className={`bg-white round-lg shadow mt-20 p-6 translate-all max-w-fit rounded-xl gap-4 overflow-auto ${
        ...
        <form className="" onSubmit={handleSubmit}>
          <div className="flex-col mx-2 my-2 text-primary py-0 space-y-1">
            ...
            <input
              ...
              ref={accountNameRef}
              onKeyDown={handleAccountNameDown}
              placeholder="Điền họ và tên của tập kết viên..."
            />
            ...
            <input
              ...
              onChange={changeAccountPhoneNumber}
              ref={accountPhoneNumberRef}
              onKeyDown={handleAccountPhoneNumberDown}
              placeholder="Điền mã của trưởng tập kết viên..."
            />
            {!accountPhoneNumberError && (
            ...
            <input
              ...
              ref={accountEmailRef}
              onKeyDown={handleAccountEmailDown}
              placeholder="Điền mã của tập kết viên..."
            />
            {!accountEmailError && (
              <p className="text-red-600">Email không đúng định dạng</p>
            )}
