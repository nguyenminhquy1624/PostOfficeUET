...
const EditTransactionAccountModal = props => {
    ...
    // }
    // const handleTransactionCodeDown = event => {
    //     if (event.key === 'Enter') {
    //         event.preventDefault();
    //         confirmRef.current.focus();
    //     }
        ...
        <div className={`bg-white round-lg shadow mt-20 p-6 translate-all max-w-fit rounded-xl gap-4 ${showEditForm ? "scale-100 opacity-100" : "scale-100 opacity-0"} overflow-auto`} onClick={(e) => e.stopPropagation()}>
            ...
            <form className='' onSubmit={handleSubmit}>
                <div className='flex-col mx-2 my-2 text-primary py-0 space-y-1'>
                    ...
                    <p>Địa chỉ/tên của điểm giao dịch</p>
                    <input className="w-full rounded-lg px-1 py-2 border-black text-black shadow-md" type='text' value={transactionName} onChange={changeTransactionName} ref={transactionNameRef} onKeyDown={handleTransactionNameDown} />
                    <div 
...
EditTransactionAccountModal.propTypes = {
